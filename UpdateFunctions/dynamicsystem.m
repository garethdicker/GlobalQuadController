...
global g m I Jr propLocation Kt Dt;
...
% compute moments
Mx = -Kt*propLocation(2,:)*(Control.rpm.^2) - state(5) * Jr * sum(rpm2rad(Control.rpm));
My =  Kt*propLocation(1,:)*(Control.rpm.^2) + state(4) * Jr * sum(rpm2rad(Control.rpm));
